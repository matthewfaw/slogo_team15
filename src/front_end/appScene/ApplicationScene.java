...
public class ApplicationScene {
    ...
    public ApplicationScene (int aWidth, int aHeight) {
        ...
        myApplicationView.add(join(myStatesBox.getInstanceAsNode(),
                                   myShapeColorModule.getInstanceAsNode()),
                              1, 1, 1, 1);
        myApplicationView
                .add(joinSwitch(myVariableViewer.getInstanceAsNode(),
                                myFunctionViewer.getInstanceAsNode(), aHeight / 2, aWidth / 4),
                     2, 1, 1, 1);
        myApplicationView.add(join(myErrorViewer.getInstanceAsNode(),
                                   myHistoryModule.getInstanceAsNode()),
                              1, 2, 2, 1);
    ...
    private Node joinSwitch (Node aVarViewer, Node aFunctionViewer, int aHeight, int aWidth) {
        ...
        javafx.collections.ObservableList<String> tabs =
                FXCollections.observableArrayList("Variables", "Functions");

        ComboBox<String> switchTabs = new ComboBox<>(tabs);
        switchTabs.getSelectionModel().selectedItemProperty().addListener( (Ov, old, neu) -> {
            column.getChildren().clear();
            if (neu.equals(var))
                column.getChildren().add(aVarViewer);
            if (neu.equals(fun))
                column.getChildren().add(aFunctionViewer);
        });
